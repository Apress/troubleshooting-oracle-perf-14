...
REM ***************************************************************************
REM
REM File name...: parallel_fts_costing_random.sql
...
BEGIN
  -- system statistics
  :cpuspeed := 2000;
  :sreadtim := dbms_random.value(1,6);
  :mreadtim := dbms_random.value(6,19);
  :mbrc := round(dbms_random.value(16,128));
...
REM
REM Test 2: no MAXTHR, SLAVETHR = 1-99% of the expected throughput of a single server process
REM

BEGIN
  :maxthr := NULL;
  :slavethr := :mreadthr*dbms_random.value(1,100)/100;
  dbms_stats.delete_system_stats();
  dbms_stats.set_system_stats(pname => 'CPUSPEED', pvalue => :cpuspeed);
  dbms_stats.set_system_stats(pname => 'SREADTIM', pvalue => :sreadtim);
  dbms_stats.set_system_stats(pname => 'MREADTIM', pvalue => :mreadtim);
...
REM
REM Test 3: MAXTHR = 1-99% of the expected throughput with dop=:max_dop, no SLAVETHR
REM

BEGIN
  :maxthr := :mreadthr*:max_dop*dbms_random.value(1,100)/100;
  :slavethr := NULL;
  dbms_stats.delete_system_stats();
  dbms_stats.set_system_stats(pname => 'CPUSPEED', pvalue => :cpuspeed);
  dbms_stats.set_system_stats(pname => 'SREADTIM', pvalue => :sreadtim);
...
REM
REM Test 4: MAXTHR = 1-99% of the expected throughput with dop=:max_dop
REM         SLAVETHR = 1-99% of the expected throughput of a single server process
REM
